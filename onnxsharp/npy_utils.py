...
def npy_summurize_array(array, name=""):
    if array is None or not isinstance(array, numpy.ndarray):
        ...
        return None
    numpy.set_printoptions(suppress=True, precision=6, sign=' ', linewidth=128, floatmode='fixed')
    ...
    if num_nan + num_inf == 0:
        print(f"norm: {numpy.linalg.norm(flatten_array)}, l2: {numpy.linalg.norm(flatten_array, ord=2)}, \n"
