...
import onnx
from onnxsharp.graph_folder import fold_graph, Encoder
        ...
        for node_info in node_infos:
            ...
            if node_info.is_subgraph:
                ...
                self.handle_layer_recursively(
                    current_namespace
                    + ("/" if current_namespace else "")
                    + f"pattern_{self.str_to_hashed_int(node_info.contained_graph_encoded)}_instance_"
                    + str(global_subgraph_index[0]),
            ...
            assert (
                node_id not in self._already_imported
            ), f"Node {node_id} already imported"
